...
x_values = pd.read_csv("training/xor_training.csv", usecols=[0,1]) 
output_values = pd.read_csv("training/xor_training.csv", usecols=[2]) 

# Putting and operator and not operator in the same tensor
x_train = torch.Tensor(x_values.values).float()
y_train = torch.Tensor(output_values.values).float()

# Correct answer: 
class XorOperatorModel:

    # For the xor-operator, the values must be initated at the start
    def __init__(self):
